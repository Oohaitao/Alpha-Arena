...
def get_hyperliquid_client(db: Session, account_id: int, override_environment: str = None) -> HyperliquidTradingClient:
    ...
    # Fallback: if no wallet for current environment, try the other environment
    if not wallet:
        fallback_environment = "testnet" if environment == "mainnet" else "mainnet"
        wallet = db.query(HyperliquidWallet).filter(
            HyperliquidWallet.account_id == account_id,
            HyperliquidWallet.environment == fallback_environment
        ).first()
        if wallet and wallet.is_active and str(wallet.is_active).lower() == 'false':
            wallet = None
        if wallet:
            logger.warning(
                f"No {environment} wallet found for account {account.name} (ID: {account_id}), "
                f"falling back to {fallback_environment} wallet. Consider configuring a {environment} wallet."
            )
    ...
    if wallet:
        ...
        # Final fallback: try other environment's key if current one is missing
        if not encrypted_key:
            fallback_env = "testnet" if environment == "mainnet" else "mainnet"
            if fallback_env == "testnet":
                encrypted_key = account.hyperliquid_testnet_private_key
            else:
                encrypted_key = account.hyperliquid_mainnet_private_key
            if encrypted_key:
                logger.warning(
                    f"No {environment} private key found for account {account.name} (ID: {account_id}), "
                    f"falling back to {fallback_env}. Consider configuring a {environment} private key."
                )
