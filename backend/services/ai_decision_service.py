...
- Default take profit: +10% from entry (adjust based on signals)"""
    ...
    # ============================================================================
    # LAST N TRADE REASON VARIABLE PROCESSING
    # ============================================================================
    # Process {last_N_trade_reason} dynamic variables where N is the number of recent trades.
    # Format: [decision_time:时间，reason:理由]
    last_trade_reason_context = {}
    if template_text:
        try:
            import re
            pattern = r'\{last_(\d+)_trade_reason\}'
            matches = re.findall(pattern, template_text)
            if matches:
                from database.connection import SessionLocal
                from sqlalchemy import text
                unique_nums = set(int(m) for m in matches)
                max_n = max(unique_nums)
                
                with SessionLocal() as trade_db:
                    result = trade_db.execute(
                        text("""
                            SELECT decision_time, reason 
                            ...
                            LIMIT :limit
                        """),
                        {"account_id": account.id, "limit": max_n}
                    )
                ...
                for row in rows:
                    ...
                    reason = row[1]
                    if decision_time:
                        time_str = decision_time.strftime('%Y-%m-%d %H:%M:%S') if hasattr(decision_time, 'strftime') else str(decision_time)
                        trade_decisions.append(f"[decision_time:{time_str}，reason:{reason}]")
                
                for n in unique_nums:
                    last_trade_reason_context[f"last_{n}_trade_reason"] = "\n".join(trade_decisions[:n]) if trade_decisions else "No trade history available"
                
                logger.debug(f"Built last N trade reason context for N values: {unique_nums}")
        except Exception as e:
            logger.warning(f"Failed to build last N trade reason context: {e}", exc_info=True)
