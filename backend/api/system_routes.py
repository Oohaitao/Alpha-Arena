...
BINANCE_DATA_ENABLED_KEY = "binance_data_enabled"

@router.post("/binance-data-toggle")
async def toggle_binance_data(enabled: bool = True, db: Session = Depends(get_db)):
    """
    Toggle Binance data collection on/off.
    
    Args:
        enabled: Set to false to disable Binance data collection, true to enable
    """
    config = db.query(SystemConfig).filter(SystemConfig.key == BINANCE_DATA_ENABLED_KEY).first()
    
    if config:
        config.value = "true" if enabled else "false"
    else:
        config = SystemConfig(
            key=BINANCE_DATA_ENABLED_KEY,
            value="true" if enabled else "false",
            description="Enable/disable Binance data collection"
        )
        db.add(config)
    
    db.commit()
    
    logger.info(f"Binance data collection {'enabled' if enabled else 'disabled'}")
    
    return {
        "enabled": enabled,
        "message": "Binance data collection " + ("enabled" if enabled else "disabled") + ". Restart required for changes to take effect."
    }


@router.get("/binance-data-status")
def get_binance_data_status(db: Session = Depends(get_db)):
    """Get current Binance data collection status."""
    config = db.query(SystemConfig).filter(SystemConfig.key == BINANCE_DATA_ENABLED_KEY).first()
    ...
    return {
        "enabled": enabled,
        "config_value": config.value if config else "true (default)"
    }
